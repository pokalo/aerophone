const CLIENT_ID = '1971299567085305024';
const CLIENT_SECRET = 'F3728720303FF0E4A6B58C5AC1E3078F9BCEBC5ECE849D5EC5B3D73369B91173';
const APP_ID = '118008397';
const DOMAIN = 'connect-api.cloud.huawei.com';

async function tryEndpoints() {
  const tres = await fetch(`https://${DOMAIN}/api/oauth2/v1/token`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ grant_type: 'client_credentials', client_id: CLIENT_ID, client_secret: CLIENT_SECRET }),
  });
  const tjson = await tres.json();
  const token = tjson.access_token;
  const headers = { 'Authorization': `Bearer ${token}`, 'client_id': CLIENT_ID };

  const endpoints = [
    `/api/publish/v2/app-privacy?appId=${APP_ID}`,
    `/api/publish/v2/privacy-tag?appId=${APP_ID}`,
    `/api/publish/v2/privacy?appId=${APP_ID}`,
    `/api/publish/v2/data-processing?appId=${APP_ID}`,
    `/api/publish/v2/app-privacy-tag?appId=${APP_ID}`,
  ];

  for (const ep of endpoints) {
    try {
      const res = await fetch(`https://${DOMAIN}${ep}`, { headers });
      const text = await res.text();
      let json;
      try { json = JSON.parse(text); } catch { json = text; }
      console.log(`GET ${ep}: ${res.status}`, JSON.stringify(json).slice(0, 300));
    } catch (e) {
      console.log(`GET ${ep}: ERROR ${e.message}`);
    }
  }
}
tryEndpoints();
